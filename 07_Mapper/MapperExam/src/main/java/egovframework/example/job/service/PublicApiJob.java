...
public class PublicApiJob {
	...
	//상수정의
	final String serviceKey ="nbDIjISLbIXSdabMCuJb9SWybZ5HWeEGp0W7AmeIixCHyGNIYlMxp14iXudn0qG55M%2FDyGRp4jb132d%2FJ%2BfVyg%3D%3D";   //공공데이터 api인증 키
	...
	public void autoInsert() throws Exception{
		...
		// 사용법:Request request = new Request.Builder().url(인터넷주소).build();
		Request request = new Request.Builder().url(url).build(); // 요청 정보
		// 3)실제 요청: 공공데이처 포털에 직접 
		// 사용법:Response response = client.newCall(요청변수).execute();
