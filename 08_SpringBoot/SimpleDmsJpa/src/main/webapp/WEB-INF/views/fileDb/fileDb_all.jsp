        ...
        <div class="col3">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    >삭제</a>
                        <%--                사용법: <sec:authentication property="name"/> : name속성명으로 email값을 가지고 있음(로그인했을떄)    --%>
...
<script type="text/javascript">
    <%-- TODO: ajax 좋아요 함수   --%>
    <%--  사용법: $.ajax({     url: "컨트롤러함수url"
                            , method:"POST"
                            , data:{속성:값} (CUD 할때, 조회시 생략)
                            , success:함수(결과)
                            , error:함수(에러)}) --%>
    <%-- ajax(csrf토큰) -> react(인증: 웹토큰) --%>
    function fn_like(email, uuid) {
        $.ajax({
            url:"/api/filedb/likes/add",
            method:"POST",
            data:{                  // dto 연결
                email:email,
                uuid:uuid,
                likeCount:1,
                '${_csrf.parameterName}':'${_csrf.token}' // csrf 토큰 전송(post 방식)
            },
            success: function (r){
                // 성공시 처리
