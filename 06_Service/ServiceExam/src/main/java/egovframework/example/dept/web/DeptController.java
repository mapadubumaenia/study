...
//   DB 저장(저장 버튼 클릭시 실행)
   ...
   public String insert(@ModelAttribute DeptVO deptVO) {
      log.info(deptVO);  //  확인(JSP 보낸것)
//      저장: 서비스 클래스의 저장함수 실행
      deptService.insert(deptVO);
      
//      저장후에 전체조회 페이지로 강제 이동(redirect): /dept/dept.do
//      강제 이동 사용법:  return "redirect:url경로";
      return "redirect:/dept/dept.do";
   }
   
//   수정 페이지 열기: JSP에서 기본키(dno)를 전달받아서 상세조회를 합니다.
   @GetMapping("/dept/edition.do")
   public String updateDeptView(Model model,
          @RequestParam(defaultValue = "0") long dno) {
      log.info("테스트: "+dno);   // dno 확인
//      서비스의 상세조회 메소드 실행 -> 결과: JSP 전달
